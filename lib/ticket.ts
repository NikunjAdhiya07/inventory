...
// Ticket numbers for completed inventory entries.
//
// Format: INV-202607-0042 — prefix, period, then a sequence that restarts each
...
export async function nextTicketNumber(db: Db, when = new Date()): Promise<string> {
  ...
  try {
    ...
    return `${PREFIX}-${period}-X${Date.now().toString(36).toUpperCase()}`;
  }
}
