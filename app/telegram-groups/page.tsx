...
export default function TelegramGroupsPage() {
  ...
  function openEdit(g: Group) {
    setEditingId(g.id);
    setForm({ chatId: g.chatId, title: g.title, manualInactive: g.manualInactive });
    setModalOpen(true);
  }
  ...
  async function toggleManual(g: Group) {
    ...
    setGroups((prev) =>
      prev.map((x) => (x.id === g.id ? { ...x, manualInactive: next, status: effStatus({ ...x, manualInactive: next }) } : x))
    );
  ...
  const openLogs = useCallback(async (g: Group, level: "all" | "error" = "all") => {
    ...
    setLogsLoading(true);
    try {
      const d = await api.get<TgLog[]>(`/api/telegram-groups/${g.id}/logs${level === "error" ? "?level=error" : ""}`);
      setLogs(d);
    } catch {
      setLogs([]);
    } finally {
  ...
  return (
    <PageShell section="Automation" page="Telegram Groups">
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap", marginBottom: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 12.5, color: "#4a5878", fontWeight: 600 }}>
          <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#0f9d63" }} />
          {activeCount} active
          <span style={{ color: "#c4ccda" }}>·</span>
          <span style={{ color: "#8a97b0" }}>{groups.length - activeCount} inactive</span>
        </div>
        <button onClick={runHealthAll} disabled={checking} style={{ ...secondaryBtnStyle, padding: "8px 14px", opacity: checking ? 0.6 : 1, cursor: checking ? "default" : "pointer" }}>
          {checking ? "Pinging bots…" : "↻ Run health check"}
        </button>
        <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5, color: "#4a5878", fontWeight: 600, cursor: "pointer" }}>
          <button onClick={() => setAutoRefresh((v) => !v)} style={toggleStyle(autoRefresh)} aria-label="Toggle auto-refresh">
            <span style={toggleKnobStyle(autoRefresh)} />
          </button>
          Auto-refresh
        </label>
        <span style={{ fontSize: 12, color: "#98a4bd" }} title={lastRefreshed?.toLocaleString()}>
      ...
      <section style={{ background: "#fff", border: "1px solid #e9edf3", borderRadius: 14, overflow: "hidden", boxShadow: "0 1px 2px rgba(16,30,54,.04)" }}>
        <div style={{ padding: "15px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, borderBottom: "1px solid #f1f4f8", flexWrap: "wrap" }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "#0b1b45" }}>All groups</div>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <select
              value={statusFilter}
              onChange={(e) => setStatusFilter(e.target.value as typeof statusFilter)}
              style={{ padding: "9px 12px", border: "1px solid #dfe5ee", borderRadius: 9, fontSize: 13, background: "#fbfcfe", color: "#3a4a68", fontWeight: 600 }}
            >
                  ...
                  <tr key={g.id}>
                    ...
                    <td style={tdStyle()}>
                      ...
                      <div style={{ marginTop: 5 }}>
                        <button
                          ...
                          style={{ border: "none", background: "none", padding: 0, fontSize: 11, fontWeight: 600, color: "#8a97b0", cursor: "pointer", textDecoration: "underline" }}
                        >
                          {g.manualInactive ? "Release override" : "Force inactive"}
                        </button>
                      </div>
                    </td>
                    <td style={{ ...tdStyle(), color: "#67748e" }}>
                      {ago(g.lastSeenAt)}
                      {g.lastError ? <div style={{ fontSize: 11, color: "#d63a3a", marginTop: 2, maxWidth: 220 }}>{g.lastError}</div> : null}
                    </td>
                    <td style={{ ...tdStyle(), padding: "12px 16px 12px 14px" }}>
                      <div style={{ display: "flex", gap: 6, justifyContent: "flex-end", flexWrap: "wrap" }}>
                        ...
                        <button onClick={() => openLogs(g)} style={actionBtnStyle("#5b57c9", "#d8d6f0")}>
                          Logs
                        </button>
        ...
        {!loading && filtered.length === 0 ? <EmptyState text={groups.length ? "No groups match your filters." : "No groups yet. Add one to assign a workflow to it."} /> : null}
      ...
      {modalOpen ? (
        <Modal onClose={() => setModalOpen(false)}>
          ...
          <div style={{ padding: "22px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 12, background: "#f8fafc", border: "1px solid #eef2f7", borderRadius: 10, padding: "12px 14px" }}>
              <button onClick={() => setF("manualInactive", !form.manualInactive)} style={toggleStyle(form.manualInactive)} aria-label="Toggle manual override">
                <span style={toggleKnobStyle(form.manualInactive)} />
              </button>
              <div>
                <div style={{ fontSize: 12.5, fontWeight: 700, color: "#3a4a68" }}>Force inactive (manual override)</div>
                <div style={{ fontSize: 11.5, color: "#8a97b0", marginTop: 2 }}>Keeps the group offline even when the bot health check passes.</div>
              </div>
            </div>
          </div>
          <ModalFooter>
      ...
      {logsGroup ? (
        <Modal onClose={() => setLogsGroup(null)} maxWidth={640}>
          <ModalHeader title={`Activity · ${logsGroup.title}`} subtitle="Inventory updates, commands, health checks and errors for this group." onClose={() => setLogsGroup(null)} />
          ...
          <div style={{ padding: "14px 24px 24px", maxHeight: 440, overflowY: "auto" }}>
            {logsLoading ? (
              <EmptyState text="Loading activity…" />
            ) : logs.length === 0 ? (
              <EmptyState text={logLevel === "error" ? "No errors logged for this group." : "No activity logged yet."} />
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {logs.map((l) => {
                  const c = logTypeColor[l.type] ?? logTypeColor.update;
                  return (
                    <div key={l.id} style={{ display: "flex", gap: 12, alignItems: "flex-start", padding: "10px 12px", border: "1px solid #f1f4f8", borderRadius: 10, background: l.level === "error" ? "#fffafa" : "#fff" }}>
                      <span style={{ flexShrink: 0, fontSize: 10.5, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".3px", padding: "3px 8px", borderRadius: 6, background: c.bg, color: c.fg }}>
                        {l.type}
                      </span>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 13, color: l.level === "error" ? "#d63a3a" : "#2b3a55", fontWeight: l.level === "error" ? 600 : 500 }}>{l.message}</div>
                        <div style={{ fontSize: 11, color: "#98a4bd", marginTop: 2 }}>{new Date(l.ts).toLocaleString()} · {ago(l.ts)}</div>
                      </div>
                    </div>
                  );
                })}
